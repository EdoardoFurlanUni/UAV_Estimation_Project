...
% % Setup % project_dir = fileparts(mfilename('fullpath'));
project_dir = pwd;
...
% % GPS - denied interval T_deny = 100;
I_deny = 40;
...
idx_gps  = setdiff(1:N-1, deny_start:deny_end);
% GPS - available steps idx_flow = deny_start : min(deny_end, N - 1);
% GPS - denied steps
% %Init
x0 = [q_sync(1, :) '; gps_gt(1,1:3)';
     gps_gt(1, 4 : 5)'; -dist_h(1); 2.7556e-6 * ones(3, 1);
     6.7600e-11 * ones(3, 1)];
...
for k = 2 : N 
    xk = veh_flow_v(k - 1, 1);
    yk = veh_flow_v(k - 1, 2);
if k < burn_in || bs(xk) < VEL_THR_x(k - 1) 
...
% % Ground truth gt_vel = gps_gt(1 : N - 1, 1 : 3);
gt_pos = [ gps_gt(1 : N - 1, 4 : 5), -dist_h(1 : N - 1) ];
calc_rmse = @(est, gt) sqrt(mean(sum((est - gt).^ 2, 2)));

% % Helper : run EKF with given R_gps and R_flow run_ekf = @(R_gps, R_flow)
    run_ekf_inner(N, x0, P0, mode_vec, gps_denied, ... veh_flow_v, baro_h,
                  dtheta, dv, Delta_theta_n, Delta_v_n, wb, ab, dt, ... R_gps,
                  R_flow, VEL_THR_x, VEL_THR_y);
...
% Base values bv_h = 0.05;
bv_d = 0.1;
% velocity std(m / s) bp_h = 0.3;
bp_d = 0.4;
% position std(m)

        gps_scales = [];
% Skipped m_gps = [ 0.5, 0.5, 0.001, 0.05, 0.05 ];
% Locked to optimal tuned values
